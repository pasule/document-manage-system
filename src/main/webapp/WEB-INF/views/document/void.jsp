...
<head>
    ...
    <style>
        ...
        .void-card { width: 480px; margin: 40px auto; background: #fff; border-radius: 10px; box-shadow: 0 2px 12px #90caf9; padding: 32px 36px; }
        ...
        .btn-submit { width: 100%; background: #f44336; color: white; border: none; padding: 12px; border-radius: 4px; cursor: pointer; font-size: 16px; }
...
<div class="void-card">
    ...
    <div class="info-group">
        <label>档案编号:</label>
        <div class="info-text">${document.code}</div>
    </div>
    
    <div class="info-group">
        <label>档案标题:</label>
        <div class="info-text">${document.title}</div>
    </div>
    
    <div class="info-group">
        <label>密级:</label>
        <div class="info-text">${document.secretLevelName}</div>
    </div>
    
    <div class="info-group">
        <label>创建时间:</label>
        <div class="info-text">${document.createTime}</div>
    </div>
    
    <form action="${pageContext.request.contextPath}/document/void" method="post">
        <input type="hidden" name="documentId" value="${document.id}">
        
        <div class="form-group">
            <label>作废原因: <span style="color: #f44336;">*</span></label>
        ...
        <button type="submit" class="btn-submit">提交作废申请</button>
    </form>
